# ============================================================================
# RANDOM FOREST - ECONOMIC CRISIS PREDICTION
# Independent Project - No Dependencies
# ============================================================================
# File: main.py (for Random Forest project)
...
def main():
    """Main execution function for Random Forest model"""
    
    print("="*100)
    print(" "*20 + "RANDOM FOREST - ECONOMIC CRISIS PREDICTION")
    ...
    print("  ✓ Random Forest trained with 500 trees")
    print("  ✓ Out-of-Bag Score: {0:.4f}".format(rf_model.oob_score_))
    ...
    y_pred_train = rf_calibrated.predict(X_train_scaled)
    y_pred_test = rf_calibrated.predict(X_test_scaled)
    y_proba_train = rf_calibrated.predict_proba(X_train_scaled)[:, 1]
    y_proba_test = rf_calibrated.predict_proba(X_test_scaled)[:, 1]
    ...
    cv_scores = cross_val_score(rf_calibrated, X_train_scaled, y_train, cv=5, scoring='roc_auc')
    ...
    feature_importance = sorted(zip(feature_names, feature_importance_scores), key=lambda x: x[1], reverse=True)
    print("    Top 5 Features:")
    for i, (feat, imp) in enumerate(feature_importance[:5], 1):
        print("      {0}. {1}: {2:.4f}".format(i, feat, imp))
    
    # Step 11: Sample Predictions
    print("\n[STEP 11] Sample Predictions...")
    sample_data = np.array([[3.5, 5.5, 2.0, 60.0, 2.5, 5.0, 2.0, 8.0, 28.0, 1.5, 10.0]])
    sample_scaled = scaler.transform(sample_data)
    sample_pred = rf_calibrated.predict(sample_scaled)[0]
    sample_proba = rf_calibrated.predict_proba(sample_scaled)[0, 1]
    ...
    print("    AUC-ROC:          {0:.4f}".format(test_auc))
    print("  ")
    print("  Best For: Robust predictions with feature importance ranking")
    print("="*100)
    
    print("\n✓✓✓ RANDOM FOREST MODEL EXECUTION COMPLETE ✓✓✓\n")
